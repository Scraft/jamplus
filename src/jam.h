...
struct globs {
	...
	int	printtarget;		/* DEBUG_MAKE prints target's
					 * name intead of target's
					 * bound filename. */
...
#ifdef OPT_BUILTIN_MD5CACHE_EXT
# define DEBUG_MD5HASH  ( globs.debug[ 15 ] ) /* -dh show md5 caches */
#endif
#ifdef OPT_GRAPH_DEBUG_EXT
# define DEBUG_GRAPH	( globs.debug_graph )	/* -dg show good dependency
						 * graph*/
# define DEBUG_FATE	( globs.debug_fate )	/* -df show fate
						 * changes */
#endif /* OPT_GRAPH_DEBUG_EXT */
#ifdef OPT_DEBUG_MEM_TOTALS_EXT
# define DEBUG_MEM_TOTALS ( globs.debug_memtotals ) /* print total memory
							allocation stats */
#endif /* OPT_DEBUG_MEM_TOTALS_EXT */
#ifdef OPT_DEBUG_MAKE1_LOG_EXT
#define DEBUG_MAKE1	( globs.debug_make1 )	/* -dM show progress of
						 * make1 */
...
#define CHECK_LEAKS() ((void) check_leaks())
#else
...
void* track_malloc(size_t mem);

#define calloc(nelem, elsize) track_calloc(nelem, elsize)
